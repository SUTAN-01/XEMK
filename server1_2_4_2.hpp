...
public:
    ...
    std::vector<std::vector<Card*>> last_slots_cards;
    ...
    void on_message(websocketpp::connection_hdl hdl, server::message_ptr msg) {
        try {
            ...
            if(choosing_card==1&&type=="special_action")
            {
                ...
                if(flag==1){
                    //记录当前玩家的信息方便下个玩家出牌结束后开始对战
                    //当前玩家的出牌信息
                
                }
                else if(flag==2){
                    //两个玩家出牌结束，结束当前回合，开始系统操作
                    Logger::info("=====================双方玩家出牌结束，结束当前回合========================");
                    //当前玩家player_idnex，对方玩家player_idnex_op，当前玩家和对方玩家与flag=1时相反
                    //注意双方玩家的卡牌变化，如变身、召唤等
   ...
   void handle_player_join(websocketpp::connection_hdl hdl, const json& data) {
    ...
    void handle_player_action(websocketpp::connection_hdl hdl, const json& data) {
        ...
        if (data.contains("slots") && data["slots"].is_array()) {
            for (const auto& slot_data : data["slots"]) {
                std::vector<Card*> slot_cards;
                for (const auto& card_data : slot_data) {
                    std::string card_name = card_data["name"].get<std::string>();
                    if(card_name == "松鼠"){
                        Card* card = cardRandomizer.getsquirrel();
                        // Card* card = new Card(card_name,card->getHP(),card->getATK(),card->getproperty(),card->getcost(),card->getrace());
                        // card->set_play_current_epoch(true);
                        slot_cards.push_back(card);
                    }
                    else{
                        Card* card = cardRandomizer.getcard(card_name);
                        // card->set_play_current_epoch(true);
                        slot_cards.push_back(card);
                    }
        ...
        played_cards = game_play.cur_play(player_id, slots_cards, card_names, cardRandomizer);
        ...
        // 通知对方玩家
        notify_opponent_move(player_id, played_cards, slots_cards);

...
public:
    ...
    std::vector<Card*> cur_play(std::string player_id, std::vector<std::vector<Card*>> &cur_slots_cards,
        std::vector<std::string> card_names,CardRandomizer &cardRandomizer){
        played_cards.clear();
        ...
        if(last_slots_cards[player_id]!=cur_player_slots_cards[player_id])
        {
            ...
            played_cards=played_cards_update(card_names, cardRandomizer);//加上了不该加的
        }
        return played_cards;
    }
    int getHP(int HP){
    ...
    std::vector<Card*> played_cards_update(std::vector<std::string> card_names, CardRandomizer &cardRandomizer){
        // 通过名称查找卡牌对象

...
public:
    ...
    std::vector<std::vector<Card*>> an_slot_card(const json& data, std::unordered_map<std::string, std::vector<Card*>> &player_cards_,
        ...
        std::vector<std::vector<Card*>> &slots_cards,int &out_card_num){
        // int out_card_num=0;
        int ind=0;
                                    ...
                                    if(out_card_num>=cost_num){
                                        ...
                                        if(cost_num>0){
                                            if(!slot_cards.empty()){//将槽位中原有的卡献祭掉
                                                std::cout<<"yuanka="<<slot_cards[0]->getName()<<std::endl;
                                                // std::cout<<"xianka="<<slot_cards[1]->getName()<<std::endl;
                                                // it = player_cards.erase(it);
                                                for(auto it1 = player_cards.begin(); it1 != player_cards.end();){
                                                    if((*it1)->get_play_current_card_id()==slot_cards[0]->get_play_current_card_id()){
                                                        (*it1)->set_card_state(0);
                                                        it1 = player_cards.erase(it1);
                                                        break;
                                                    }
                                                }
                                                slot_cards.erase(slot_cards.begin());
                                                player_bones+=1;
                                                (*it)->reducecost(1);

                ...
                if (type == "player_join") {
                    ...
                    if(player_idnex!=last_player){
                        if(payload["action"]=="clear"){
                            int xianjiing=0;
                            for(auto &card:player_cards_[player_idnex]){
                                if(card->get_card_state()==2){
                                    xianjiing=1;
                                }
                            }
                            if(xianjiing==1){
                                ...
                                for(auto &&it=player_cards_[player_idnex].begin();it!=player_cards_[player_idnex].end();it++){
                                    if((*it)->get_play_current_card_id()==xj_card_id&&(*it)->getHP()>0){
                                        // (*it)->set_card_state(1);
                                        // while((*it)->getHP()>0){
                                            (*it)->lossHP(100);
                                        // }
                                ...
                                for(auto &card:player_cards_[player_idnex]){
                                    if(card->get_card_state()==2){
                                        if(card->getcost().begin()->second!=0){
                                            card->reducecost(1);
                                            break;
                                        }
                                    }
                            ...
                            int xj_card_id=payload["card"]["card_id"];
                            for(auto &card:player_cards_[player_idnex]){
                                if(card->get_play_current_card_id()==xj_card_id){
                                    card->set_card_state(1);
                                    // break;
                                }
                            }
                        }

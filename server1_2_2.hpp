...
//game_interface1_2_2.html，基本功能实现，但还保留鸽子，并且鸽子功能不完善
// 自定义日志函数替代ROS的日志
class Logger {
public:
...
private:
   ...
   void handle_player_join(websocketpp::connection_hdl hdl, const json& data) {
    ...
    void handle_player_action(websocketpp::connection_hdl hdl, const json& data) {
        ...
        std::vector<std::vector<Card*>> slots;  // 栏位数据
        if (data.contains("slots") && data["slots"].is_array()) {
            slots.resize(4);  // 确保有4个栏位
            
            for (size_t i = 0; i < data["slots"].size() && i < 4; i++) {
                const auto& slot_array = data["slots"][i];
                
                if (slot_array.is_array()) {
                    for (const auto& card_json : slot_array) {
                        // slots[i].push_back(CardData(card_json));
                    }
                }
            }
        }

...
#include "card3_2_1.hpp"
#include "play3_2_1.hpp"
    ...
    void on_message(websocketpp::connection_hdl hdl, server::message_ptr msg) {
        try {
            ...
            if(choosing_card==1&&type=="special_action")
            {
                ...
                }else if(type=="choosing_card_sacrifice"){
    ...
    std::vector<std::vector<Card*>> handle_player_action(websocketpp::connection_hdl hdl, const json& data) {
       ...
       //其中在CardRandomizer类中给出了卡的模板，玩家每次获取的卡是模板的复制卡
        ...
        if(flag==1&&last_slots_cards[player_idnex].size()!=0){
            ...
            // for(auto &cards:player_cards_[player_idnex]){//需要确定代码有无错误
            //     if(cards->get_card_state()==0){
            //         auto cost=cards->getcost().begin();
            //         if(cost==nullptr){
            //             in_card_num++;
            //         }
            //     }
            ...
            for(int i=0;i<sss;i++)
            {
                if(slots_cards[i].size()>0&&slots_cards[i][0]->getHP()>0){
                    out_card_num++;//场上可供献祭的牌数量
                }
            }
        ...
        //                     if(costit!=nullptr&&state==0){
        //                         std::string cost_name=costit->first;
        //                         int cost_num=costit->second;
        //                         if(costit->first=="血滴"){
        //                             if(out_card_num>cost_num){
        //                                 int need_num=cost_num;
        //                                 while (need_num>0){
        //                                     if(!slot_cards.empty()){//将槽位中原有的卡献祭掉
        //                                         // it = player_cards.erase(it);
        //                                         for(auto it1 = player_cards.begin(); it1 != player_cards.end();){
        //                                             if((*it1)->get_play_current_card_id()==slot_cards[0]->get_play_current_card_id()){
        //                                                 it1 = player_cards.erase(it1);
        //                                                 break;
        //                                             }
        //                                         }
        //                                         slot_cards.clear();
        ...
        slots_cards=game_play.an_slot_card(data, player_cards_, cardRandomizer, 
            card_id, player_idnex, player_bones, slots_cards,out_card_num);

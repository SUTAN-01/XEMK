                ...
                if (type == "player_join") {
                    ...
                    {   
                        ...
                        if (flag == 1) {
                            ...
                            std::vector<std::vector<Card*>> last_player_slots_cards=slotscards[0];
                            std::vector<std::vector<Card*>> cur_player_slots_cards=slotscards[1];
                            for(auto &card:cur_player_slots_cards){
                                if(card.size()!=0&&(card[0]->getHP())<=0){
                                    auto it=std::find(last_player_slots_cards.begin(),last_player_slots_cards.end(),card);
                                    if(it!=last_player_slots_cards.end()){
                                        Logger::info("last_player_slots_cards!!!!!!!!!!!!!!!");
                                        it=last_player_slots_cards.erase(it);
                                    }
                                }
                            }
                            for(auto &card:cur_player_slots_cards){
                                if(card.size()!=0&&card[0]->getHP()<=0){
                                    auto it=std::find(cur_player_slots_cards.begin(),cur_player_slots_cards.end(),card);
                                    if(it!=cur_player_slots_cards.end()){
                                        Logger::info("cur_player_slots_cards!!!!!!!!!!!!!!!");
                                        it=cur_player_slots_cards.erase(it);
                                    }
                                }
                            }
                            if(game_end==-1){
                            ...
                            // 通知对方玩家
                            notify_opponent_move(player_idnex, cur_player_slots_cards);
                            notify_opponent_move(player_idnex_op, last_player_slots_cards);
                            // 立即发送新卡牌
                            // send_cards_to_player(player_idnex);
                            // send_cards_to_player(player_idnex_op);
                            choosing_card=1;
   ...
   void handle_player_join(websocketpp::connection_hdl hdl, const json& data) {
    ...
    std::vector<std::vector<Card*>> handle_player_action(websocketpp::connection_hdl hdl, const json& data) {
       ...
       //其中在CardRandomizer类中给出了卡的模板，玩家每次获取的卡是模板的复制卡
        ...
        if (data.contains("slots") && data["slots"].is_array()) {
            ...
            for (const auto& slot_data : data["slots"]) { // data["slots"]中含有四个slot_data
                ...
                for (const auto& card_data : slot_data) { // slot_data中包含多个card_data
                    ...
                    auto& player_cards = player_cards_[player_id];
                ...
                slots_cards.push_back(slot_cards);
            }
        }
        ...
        // 通知对方玩家
        notify_opponent_move(player_id, slots_cards);
